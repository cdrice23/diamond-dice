...
export function AnimatedSlot({ position, visible, moveDelay = 0, fadeDelay = 0, fadeDuration = FADE_TRANSITION_DURATION, errorText, children }: AnimatedSlotProps) {
  ...
  return (
    <Animated.View style={[{ position: 'absolute', top: 0, left: 0, right: 0, height: SLOT_HEIGHT }, animatedStyle]} pointerEvents={visible ? 'auto' : 'none'}>
      {children}
      {errorText && <Text className="text-level3 text-xs mt-1">{errorText}</Text>}
    </Animated.View>
  );
}