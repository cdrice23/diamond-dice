...
def get_levels() -> list[dict]:
    result = (
        get_client()
        .table("levels")
        .select("level, min_avg, max_avg, min_era, max_era")
        .execute()
    )
    return result.data
...
def record_failed_player(player_id: str, error: str) -> None:
    get_client().table("etl_failed_players").upsert(
        {"player_id": player_id, "last_error": error, "last_attempted_at": "now()"},
        on_conflict="player_id",
    ).execute()


def clear_failed_player(player_id: str) -> None:
    get_client().table("etl_failed_players").delete().eq(
        "player_id", player_id
    ).execute()
...
def get_config_value(key: str, default: str | None = None) -> str | None:
    result = (
        get_client().table("system_config").select("value").eq("key", key).execute()
    )
    if result.data:
...
def upsert_team_tenure(
    player_uuid: str, team_uuid: str, start_date: str, end_date: str
) -> None:
    row = {
        "player_id": player_uuid,
        "mlb_team_id": team_uuid,
        "start_date": start_date,
        "end_date": end_date,
    }
    get_client().table("player_mlb_team_history").upsert(
        row, on_conflict="player_id,mlb_team_id,start_date"
    ).execute()
...
def get_player_ids_missing_team_history() -> list[str]:
    ...
    history_rows = (
        get_client().table("player_mlb_team_history").select("player_id").execute().data
    )
