drop function if exists get_teams_list(text, uuid, text, int, int);
...
as $$
  ...
  where t.owner_id = auth.uid()
    and (p_search_term is null or unaccent(t.team_name) ilike unaccent('%' || p_search_term || '%'))
    and (p_format_id is null or t.format_id = p_format_id)
  order by
    case when p_sort_direction = 'asc' then t.updated_at end asc,
    case when p_sort_direction = 'desc' then t.updated_at end desc
