...
AS $$
  SELECT
    p.id, p.name, p.eligible_positions,
  ...
  WHERE (p.is_qualified_batter OR p.is_qualified_pitcher)
    ...
    AND (
      ...
      OR (player_type = 'batter' AND p.batting_rating_level = ANY(rating_levels))
      OR (player_type = 'pitcher' AND p.pitching_rating_level = ANY(rating_levels))
      OR (player_type IS NULL AND (p.batting_rating_level = ANY(rating_levels) OR p.pitching_rating_level = ANY(rating_levels)))
    )
    AND (
