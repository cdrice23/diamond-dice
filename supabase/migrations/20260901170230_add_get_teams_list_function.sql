...
as $$
  ...
  select
    ...
    coalesce(
      (
        select jsonb_agg(jsonb_build_object('id', r.player_id, 'name', r.player_name, 'image_url', r.image_url))
        from roster r
    ...
    jsonb_build_object(
      ...
      'OF', coalesce(
        (select jsonb_agg(r.batting_rating_level) from roster r where r.team_id = t.id and r.current_position = 'OF'),
        '[]'::jsonb
      )
    ) as position_levels,
    coalesce(
      (select jsonb_agg(r.pitching_rating_level) from roster r where r.team_id = t.id and r.current_position = 'P'),
      '[]'::jsonb
    ) as pitcher_levels,
    coalesce(
      (
